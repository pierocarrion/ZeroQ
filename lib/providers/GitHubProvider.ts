...
export class GitHubProvider implements SourceProvider {
  ...
  private headers(token?: string): Record<string, string> {
    const h: Record<string, string> = { Accept: "application/vnd.github+json", "User-Agent": "zeroq" };
    const t = token ?? config.github.token;
    if (t) h.Authorization = `Bearer ${t}`;
    return h;
  }

  async validateOrg(org: string, _token?: string): Promise<{ login: string; avatar_url?: string } | null> {
    const res = await fetch(`${config.github.api}/orgs/${encodeURIComponent(org)}`, { headers: this.headers(), cache: "no-store" });
  ...
  async loadRepository(target: Target, maxFiles: number): Promise<LoadedRepository> {
    ...
    if (meta.status === 404) throw new ProviderError("Repository not found (must be public).");
    if (meta.status === 403) throw new ProviderError("GitHub rate limit hit (60 req/hr unauthenticated). Try again later or set GITHUB_TOKEN.");
  ...
  private async fetchFiles(owner: string, repo: string, branch: string, paths: string[]): Promise<SourceFile[]> {
    ...
    for (let i = 0; i < paths.length; i += BATCH) {
      ...
      const results = await Promise.all(slice.map(async (path) => {
        try {
          const raw = await fetch(`https://raw.githubusercontent.com/${owner}/${repo}/${branch}/${path}`, { headers: { "User-Agent": "zeroq" }, cache: "no-store" });
